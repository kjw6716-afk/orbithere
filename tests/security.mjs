...
await denied('admin clients cannot forge provider delivery events','authenticated',ADMIN,deliverySQL);
await denied('anonymous callers cannot read delivery recipients','anon',null,'select admin_delivery_alerts()');
await denied('members cannot read delivery recipients','authenticated',U,'select admin_delivery_alerts()');
await denied('admins cannot read the private delivery table directly','authenticated',ADMIN,'select * from orbit_ops_private.delivery_events');
await as('service_role',null,deliverySQL);
await as('service_role',null,deliverySQL);
const delivery = (await as('authenticated',ADMIN,'select admin_delivery_alerts() as data')).rows[0].data;
check('signed-event retries are idempotent and admins see one unread alert',delivery.unread===1&&delivery.events.length===1&&delivery.events[0].recipient==='user@example.test');
await denied('members cannot acknowledge delivery alerts','authenticated',U,`select acknowledge_delivery_event('evt-test','user@example.test')`);
await as('authenticated',ADMIN,`select acknowledge_delivery_event('evt-test','user@example.test')`);
await as('service_role',null,deliverySQL);
check('provider retry does not reopen an acknowledged event',(await as('authenticated',ADMIN,'select admin_delivery_alerts() as data')).rows[0].data.unread===0);
await as('service_role',null,`select record_delivery_event('old','aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa','user@example.test','email.bounced','',now()-interval '31 days')`);
check('old provider replays do not extend retention',(await as('authenticated',ADMIN,'select admin_delivery_alerts() as data')).rows[0].data.events.length===1);
await db.exec(`reset role; update orbit_ops_private.delivery_events set occurred_at=now()-interval '31 days'`);
