...
- **소개·문의**: 서비스와 계산 범위 안내 · 제작자 Gmail 문의 · 기존 질문 게시판 연결
...
3. Authentication → Sign In / Providers에서 **Allow new users to sign up**과 **Allow anonymous sign-ins**를 활성화합니다. 익명 인증은 조회수·리액션·신고와 기존 작성자 연결에 사용합니다. 새 글·댓글·사진은 이메일 또는 Google 회원만 등록하며, 가입·로그인 설정은 [회원 운영 문서](docs/members.md)를 따릅니다.
...
제목을 눌러 내용·사진·댓글을 읽는 게시판입니다. 관리자 공지는 최대 3개를 상단에 고정할 수 있습니다.
목록·공지·본문에 조회수를 표시하며, 실제 상세 열기를 같은 인증 계정/글 기준 30분마다 한 번 집계합니다. 조회수 저장에는 `20260913095912_post_view_counts.sql`이 필요합니다.
...
| `test:news` | 공식 RSS 파싱·도메인 제한·XML 엔티티 거부·출처별 캐시 보존·전체 장애 복구 |
...
## 운영자 Gmail 브리핑과 제작자 문의

Work 예약 작업 `ORBIT 아침 Gmail 브리핑`은 한국 시간 아침 8시 무렵 어제 방문 증감·중요 뉴스·새 글·검토할 초안을 확인합니다. 변화가 있으면 운영자 본인 Gmail로 한 통을 보내고 `ORBIT 브리핑` 라벨을 붙입니다. 동일 날짜의 발송 메일을 확인해 중복 발송을 피합니다. 실행 규칙과 복구는 [`_editorial/MORNING_BRIEF.md`](_editorial/MORNING_BRIEF.md)를 참고하세요.

GSC Wizard의 ORBIT 도메인 속성에서 최신 집계 완료일의 Google 검색 클릭·노출, 최근 7일과 이전 7일 비교, 대표 검색어·페이지를 함께 확인합니다. 검색의 태평양 시간 기준일은 방문 집계의 KST와 구분하며, 미완료·누락 자료를 0으로 바꾸지 않습니다. 같은 검색 자료를 새 변화로 반복 알리지 않고, GSC만 실패해도 기존 브리핑은 계속합니다. 실제 검색 수치·검색어는 공개 저장소에 남기지 않습니다.

`about.html#contact`의 제작자 문의 버튼은 받는 사람과 제목을 입력한 Gmail 작성 화면을 새 탭에서 엽니다. ‘다른 메일 앱으로 문의하기’ 링크는 일반 메일 앱을 지원합니다. 화면에는 이메일 주소를 표시하지 않지만 문의 링크와 작성 화면에는 수신 주소가 남습니다. 방문자가 내용을 작성하고 직접 전송하며, 사이트에서 메일을 자동 발송하거나 Gmail 계정 권한을 요청하지 않습니다.

문의 링크는 `[ORBIT 문의]` 제목을 미리 입력합니다. Work의 `ORBIT 문의 라벨 분류` 예약은 이 표식이 있는 새 수신 메일을 확인해 `ORBIT 문의` 라벨만 붙입니다. 본문·첨부파일을 읽거나 자동 회신하지 않고 읽음 여부·받은편지함·스팸 분류를 유지합니다. Gmail 자체 필터를 만든 것은 아니므로 연결과 Work 예약이 유지되어야 합니다. 발신자가 제목에서 문의 표식을 없애면 자동 분류 대상에서 제외됩니다.
