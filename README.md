...
| `main.html` | 커뮤니티 홈 — 밤하늘 달력 · 오늘 밤 행성 · 글 남기기 세 탭, 궤도 진입(닉네임) 대화상자 |
| `sky.html` | 밤하늘 달력 — 유성우 · 일식 · 월식 · 슈퍼문 일정, 달 위상 계산 |
| `planets.html` | 오늘 밤 행성 — 행성의 방향·고도·밝기를 그날그날 계산해 보여주는 관측 도구 |
| `lounge.html` | 글 남기기 — 궤도(채널)별 글·답·리액션·신고 (Supabase) |
| `admin.html` | 관제실 — 운영자 로그인, 신고함, 업데이트 기록 |
| `terms.html` / `privacy.html` | 이용약관 · 개인정보처리방침 |
| `404.html` | 커스텀 404 |

`sky.html`의 천문 현상 일정은 하드코딩이라 2027-08 이후로는 비지만, `planets.html`은
행성 궤도요소로 매번 계산하므로 몇 년 뒤에 열어도 그날의 하늘이 그대로 나옵니다.
...
궤도(채널)는 **관측 후기 · 장비 · 실시간 하늘 · 질문 · 자유게시판** 다섯 개이고, 첫 진입 기본 채널은
**관측 후기**입니다. 나머지 넷은 `minor`로 표시해 탭을 한 톤 낮춰 둡니다.
