...
- 첫 화면에만 성운을 표시합니다. 별도 고정 이미지 레이어이므로 동작 줄이기 설정에서도 배경은 남고 유성은 사라집니다. 일반 설정에서는 `landing-meteors.js`가 20~45초의 무작위 대기 후 상단의 무작위 위치에서 유성 하나를 1.6~2.4초 동안 표시합니다. 화면 안쪽으로 짧게 이동하며, 이전 유성이 끝나야 다음 대기를 시작합니다. 비활성 탭·동작 줄이기·페이지 이탈 시 유성과 타이머를 정리하고, 복귀하면 새 대기 시간부터 시작합니다.
