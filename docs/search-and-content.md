...
- 메인 셸의 달력·행성·게시판 메뉴도 각 독립 페이지를 가리키는 실제 링크를 제공합니다. 일반 클릭은 기존 패널을 전환하고, 새 탭 열기나 JavaScript 미사용 시에는 독립 페이지로 이동합니다. 홈 하단에서도 행성 도구와 별지도 설명으로 직접 연결합니다. 이는 내부 링크 개선이며, Google의 색인 완료를 의미하지 않습니다.
