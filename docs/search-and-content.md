...
같은 날 운영자가 GSC Wizard를 연결한 뒤 `sc-domain:orbithere.com` 도메인 속성과 실제 Google 검색 실적 조회를 확인했습니다. 최신 집계 완료 날짜·클릭·노출·검색어·페이지 조회를 기존 아침 Gmail 브리핑에 반영했습니다. 실제 수치와 검색어는 공개 저장소에 남기지 않습니다. 검색 실적 조회 성공만으로 모든 페이지의 색인이나 사이트맵 제출 상태가 확인되는 것은 아닙니다. 실행 규칙은 [`_editorial/MORNING_BRIEF.md`](../_editorial/MORNING_BRIEF.md)를 참고하세요.
