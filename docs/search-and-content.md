...
5. 네이버 서치어드바이저는 2026-09-13 소유 확인과 사이트맵 제출을 완료했습니다. 제출 당시 사이트맵에는 공개 대표 URL 16개가 있으며, 수집·색인 상태를 수일 간격으로 확인합니다.
6. `https://orbithere.com/rss.xml`은 검토 후 공개된 우주 이야기의 전체 본문과 URL을 최신순으로 제공합니다. 배포를 확인한 뒤 네이버의 RSS 제출란에 한 번 제출하고 반복 제출하지 않습니다.
...
2026-09-13 기존 아침 브리핑 예약에 위 다섯 URL의 후속 비교를 반영했습니다. GSC Wizard 검사 기록에서 각 주소의 최신 결과를 읽고, 기록이 없거나 72시간 이상 지난 주소만 조회합니다. 상태의 의미 있는 변화만 보고하며 수치·검사 결과는 공개 저장소에 기록하지 않습니다. URL 검사는 색인 생성 요청을 실행하지 않습니다.
...
- 기존 관측 가이드에 하늘 상태별 대상 선택, 방향·고도 점검 순서, 관측 기록 작성 예시를 보강했습니다. 실제 관측 후기와 작성용 예시를 구분하며 행성·달력·별지도 설명으로 직접 연결합니다. 가이드의 실제 내용 변경일만 사이트맵에 반영합니다.
...
- 우주 이야기 발행기가 공개 HTML·사이트맵과 함께 RSS를 자동 갱신합니다. 홈페이지와 이야기 페이지에는 RSS 자동 발견 링크가 있습니다.
