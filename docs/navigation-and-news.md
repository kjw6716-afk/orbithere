...
- 너비 861px 이상에서는 기존 뉴스 위젯 한 개를 왼쪽 메뉴의 ‘소개·문의’ 아래로 옮깁니다. 기본 2칸, 높이 800px 이하에서는 1칸, 모바일 게시판은 본문 아래 1칸입니다. 8초 간격·650ms 세로 이동, 한 칸일 때 220ms 전환과 hover/포커스/동작 줄이기 보호를 유지합니다. 화면 너비를 바꿔도 같은 위젯을 옮겨 재시도·순환 상태를 유지합니다.
