...
MENU = [
    ...
    ('notes', '📝', '관측 노트', 'notes.html'),
    ('about', 'ⓘ', '소개·문의', 'about.html'),
]
PAGES = {'main': 'planets', **{key: key for key, *_ in MENU},
         'reading-sky': 'guide', 'terms': None, 'privacy': None, 'admin': None}
...
def render(page, active):
    ...
    for key, icon, label, href in MENU:
        ...
        if page == 'main' and key in ('sky', 'planets', 'lounge'):
            ...
            rows.append(f'<a class="nav-item nav-link{selected}" href="{href}"{aria}>{content}</a>')
    return '\n'.join([
        '<!-- orbit-navigation:start -->',
        '<aside class="sidebar orbit-navigation">',
        '<a class="side-logo orbit-brand" href="index.html" aria-label="ORBIT 홈">ORBIT.</a>',
...
def main():
    ...
    for page, active in PAGES.items():
        ...
        updated = re.sub(PATTERN, lambda _: render(page, active), text, flags=re.S)
        if updated != text:
